...
# ── NAT Gateways — one per AZ (required for 99.9% SLO; single NAT = SPOF) ───
...
resource "aws_security_group" "eks_nodes" {
  ...
  description = "EKS worker nodes — inter-node + ALB → nodes"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "rds" {
  ...
  description = "RDS — accept only from EKS nodes"
  vpc_id      = aws_vpc.main.id
...
# ── VPC Endpoints (keeps S3 + ECR traffic inside VPC) ────────────────────────
...
resource "aws_security_group" "vpc_endpoints" {
  ...
  description = "Interface VPC endpoints — allow HTTPS from nodes"
  vpc_id      = aws_vpc.main.id
